...
mod request {
    ...
    pub(crate) struct UploadPayload {
        pub(crate) chartData: super::ChartSchema,
        pub(crate) title: Option<String>,
        pub(crate) description: Option<String>,
    }
}
...
mod response {
    ...
    pub(crate) struct ChartCreationReturn {
        chartDocument: super::ChartDocument,
        publicLink: String,
    }
    impl TryInto<crate::RemoteNapchart> for ChartCreationReturn {
        type Error = ErrorKind;
        fn try_into(self) -> Result<crate::RemoteNapchart> {
            let mut res: crate::RemoteNapchart = self.chartDocument.try_into()?;
            res.public_link = Some(self.publicLink);
            Ok(res)
        }
    }
}
